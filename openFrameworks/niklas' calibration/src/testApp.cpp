...
const float diffThreshold = 5.0; // maximum amount of movement between successive frames (must be smaller in order to add a board)
...
const int startCleaningCamera = 15; // start cleaning outliers after this many samples (10 is ok...). Should be < than preCalibrateCameraTimes
const float maxErrorCamera=3.0;

const float maxErrorProjector=1.0;
const int startCleaningProjector =7.4;
const int startDynamicProjectorPattern=20; // after this number of projector/camera calibration, the projection will start following the 
...
const int minNumGoodBoards=10; // after this number of simultaneoulsy acquired "good" boards, IF the projector total reprojection error is smaller than a certain threshold, we end calibration (and move to AR mode automatically)
...
void testApp::setup(){
  ...
  // Font:
	...
	camera.setExposure(30000);
	//camera.setROI( ofRectangle(0,0,640,480));
    ...
    cout << camera.getDeviceSpecification().toString();
 	...
 	imitate(undistorted, camera);
	imitate(previous, camera);
	imitate(diff, camera);
	dispimg.resize(512,512);
...
void testApp::update(){
    try
	{
		if(camera.isFrameNew()) {		
			Mat camMat = toCv(camera); // current image to process
			...
			switch(stateCalibration) {
				...
				case CAMERA_AND_PROJECTOR_PHASE1: 
					...
					else if (calibrationProjector.size()>startDynamicProjectorPattern) dynamicProjection=false; // note that dynamic projection can be set manually too, or using this threshold on the number of boards. 
			 ...
			 //           }
					...
					if (active)
					{
						...
						if (semiFixedMode) {
                                ...
                                unsigned short widthImagePlane=1280;//calibrationProjector.getImagerResolution().width;
                                unsigned short heightImagePlane=800;//calibrationProjector.getImagerResolution().height;
...
void testApp::draw(){
    ...
    int posTextY=512+20, posTextX=10;
 
	// Draw current acquired image:
    ofSetColor(255);
	camera.update();
    camera.draw(512,0, -512, 512);
    ...
    calibrationCamera.drawPreprocessedImage(512, 0, 512, 512);
    calibrationProjector.drawPreprocessedImage(512, 512, 512, 512);
    
    // Signal the acquisition of a new board:
    if (newBoardAquired) {
        drawHighlightString(" *** NEW BOARD ACQUIRED ***", 1024+20, 512-40, cyanPrint,  ofColor(255));
    }
    
    
    // Draw detected points (for CAMERA CALIBRATION BOARD):
    calibrationCamera.drawCandidateImagePoints(512,0, -512, 512, ofColor(255,0,0));
    // Draw reprojected points (for CAMERA CALIBRATION BOARD):
    calibrationCamera.drawCandidateReprojection(512,0, -512, 512, ofColor(255,255,0));
    // Draw axis of detected board:
    calibrationCamera.drawCandidateAxis(512,0, -512, 512);
    ...
    switch(stateCalibration) {
        ...
        case CAMERA_AND_PROJECTOR_PHASE2:
            ...
            if (calibrationCamera.isReady()&&calibrationProjector.isReady()&&(calibrationProjector.size()>0)) {
                ...
                for (int i=0; i<3; i++) {
                    ...
                    extrinsics += ofToString(transCamToProj.at<double>(i,0),3)+" \n";
                }
        ...
        case AR_DEMO:
            ...
            for (int i=0; i<3; i++) {
                ...
                extrinsics += ofToString(transCamToProj.at<double>(i,0),4)+" \n";
            }
            drawHighlightString(extrinsics, posTextX, posTextY+100, yellowPrint, ofColor(0));
...
void testApp::mouseMoved(int x, int y ){
	if (stateCalibration == CAMERA_AND_PROJECTOR_PHASE2) {
		                             unsigned short widthImagePlane=1024;//calibrationProjector.getImagerResolution().width;
                                unsigned short heightImagePlane=768;//calibrationProjector.getImagerResolution().height;
                                unsigned short widthPattern=calibrationProjector.myPatternShape.getPatternSize().width;
                                unsigned short heightPattern=calibrationProjector.myPatternShape.getPatternSize().height;

		calibrationProjector.setCandidateImagePoints(x, y);
	}
}
